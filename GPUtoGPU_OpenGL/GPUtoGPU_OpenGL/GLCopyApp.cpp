...
bool GLCopyApp::init(unsigned int uiWidth, unsigned int uiHeight, const char *pClassName, std::string& strErrorMessage)
{
    ...
    if (!ADLagent::InitClass())
    {
        strErrorMessage = "ERROR, Could not init GLWindow";
        return false;
    }

    if (ADLagent::getNumGPUs() < 2)
    {
    ...
    if ((ADLagent::getNumDisplaysOnGPU(0) == 0) || ADLagent::getNumDisplaysOnGPU(1) == 0)
    {
    ...
    unsigned int uiDsp = ADLagent::getDisplayOnGPU(0, 0);
    ...
    uiDsp = ADLagent::getDisplayOnGPU(1, 0);
